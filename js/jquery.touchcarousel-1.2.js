...
(function($) {
	function TouchCarousel(element, options) {	
		...
		if(this.hasTouch || this.settings.useWebkit3d) {
			...
			if(('WebKitCSSMatrix' in window && 'm11' in new WebKitCSSMatrix())) {
				...
				if (this.settings.direction == 'vertical') {
					this._dragContainer.css({'-webkit-transform-origin':'0 0', '-webkit-transform': 'translateX(0)'});
				} else {
		...
		if(this._useWebkitTransition) {
			...
			this._yProp = '-webkit-transform';
			this._yPref = 'translate3d(0,';
			this._ySuf = 'px, 0)';
		...
		itemsJQ.each(function(index) {
			...
			itemObj.posY = currPosY;
			itemObj.width = (jqItem.outerWidth(true) || self.settings.itemFallbackWidth);
			itemObj.height = (jqItem.outerHeight(true) || self.settings.itemFallbackHeight);
			currPosX += itemObj.width;
			currPosY += itemObj.height;
		...
		this._maxYPos = this._totalItemsHeight = currPosY;
		...
		if(this.settings.keyboardNav) {
			$(document).bind("keydown.touchcarousel", function(e) {
				if(!self._blockClickEvents) {
					if ((self.settings.direction == 'horizontal' && e.keyCode === 37) || (self.settings.direction == 'vertical' && e.keyCode === 38)) {
						self.prev();
					}
					else if ((self.settings.direction == 'horizontal' && e.keyCode === 39) || (self.settings.direction == 'vertical' && e.keyCode === 40)) {
						self.next();
					}
				}
			});
	...
	TouchCarousel.prototype = {
			...
			goTo:function(id, fromAutoplay) {
				...
				if(newItem) {					
					...
					if (this.settings.direction == 'vertical') {
						this.endPos = this._getYPos();
						var newY = -newItem.posY;
						if(newY > 0) {
							newY = 0;
						} else if(newY < this.carouselHeight - this._maxYPos) {
							newY = this.carouselHeight - this._maxYPos;
						}
						this.animateTo(newY, this.settings.transitionSpeed, "easeInOutSine");
					} else {
			...
			next:function(fromAutoplay) {
				var currPos = this.settings.direction == 'vertical' ? this._getYPos() : this._getXPos();
				var newItemId = this._getItemAtPos(currPos).index;

				if(!this._pagingEnabled) {
					newItemId = newItemId + this._itemsPerMove;						
					if(this.settings.loopItems) {
						if(this.settings.direction == 'horizontal' && currPos <= this.carouselWidth - this._maxXPos) {
							newItemId = 0;
						}
						if(this.settings.direction == 'vertical' && currPos <= this.carouselHeight - this._maxYPos) {
							newItemId = 0;
						}
					}
				...
				if (this.settings.onNext) this.settings.onNext.call(this, newItemId);
			...
			prev:function(fromAutoplay) {
				...
				if (this.settings.onPrev) this.settings.onPrev.call(this, newItemId);

				this.goTo(newItemId, fromAutoplay);
			},
			getCurrentId:function() {
				var curPos = this.settings.direction == 'vertical' ? this._getYPos() : this._getXPos();
				var currId = this._getItemAtPos(curPos).index;
				return currId;
			},
			...
			setYPos:function(pos, isScrollbar) {
				if(!isScrollbar) {
					this._dragContainerStyle[this._yProp] = (this._yPref + pos + this._ySuf);
				} else {
					this._scrollbarStyle[this._yProp] = (this._yPref + pos + this._ySuf);
				}
			},
			...
			updateCarouselSize:function(leavePos) {
				...
				this.carouselHeight = this.carouselRoot.height();
					...
					if(this._pagingEnabled) {					
						...
						if(freeItems > 0) {
							for(var i = this.numItems - freeItems; i < this.numItems; i++) {								
								...
								lastItemsHeight += this.items[i].height;
							}
						} else {
							lastItemsWidth = this.carouselWidth;
							lastItemsHeight = this.carouselHeight;
						}
						
					} else {
						lastItemsWidth = this.items[this.numItems - 1].width;
						lastItemsHeight = this.items[this.numItems - 1].height;
					}
					this._maxXPos = this._totalItemsWidth + this.carouselWidth - lastItemsWidth;
					this._maxYPos = this._totalItemsHeight + this.carouselHeight - lastItemsHeight;
				} else {
					
					this._maxXPos = this._totalItemsWidth;
					this._maxYPos = this._totalItemsHeight;
				}
				
				
				if(this.settings.scrollbar) {
					if (this.settings.direction == 'vertical') {
						var scrlHeight = Math.round(this._scrollbarHolder.height() / (this._maxYPos / this.carouselHeight));
						this.scrollbarJQ.css('height', scrlHeight);
						this._scrollbarDist = this._scrollbarHolder.height() - scrlHeight;
					} else {
				...
				if(!this.settings.scrollToLast) {
					if(this.settings.direction == 'horizontal' && this.carouselWidth >= this._totalItemsWidth) {
						...
						this.setXPos(0);						
						return;
					} else if(this.settings.direction == 'vertical' && this.carouselHeight >= this._totalItemsHeight) {
						this._wasBlocked = true;
						if(!this.settings.loopItems) {
							this._arrowRightBlocked = true;
							this.arrowRight.addClass("disabled");
				...
				if(!leavePos) {
					...
					if (this.settings.direction == 'vertical') {
						var newY = this.endPos = this._getYPos();
						...
						this.animateTo(newY, 300, "easeInOutSine");
					} else {
			...
			animateTo:function(pos, speed, easing, bounceAnim, endPos, bounceSpeed, bounceEasing) {		
				...
				var scrollbarEnabled = this.settings.scrollbar,
					prop = this.settings.direction == 'vertical' ? self._yProp : self._xProp,
					pref = this.settings.direction == 'vertical' ? self._yPref : self._xPref,
					suf = this.settings.direction == 'vertical' ? self._ySuf : self._xSuf,
				...
				if(scrollbarEnabled) {
					...
					if (this.settings.direction == 'vertical') {
						sbAnimateDist = self._maxYPos - self.carouselHeight;
					} else {
						sbAnimateDist = self._maxXPos - self.carouselWidth;
					}
			...
			_getItemAtPos:function(pos) {
				...
				for(var i = 0; i < self.numItems; i++) {					
					...
					if((this.settings.direction == 'horizontal' && pos >= currItem.posX && pos < currItem.posX + currItem.width) ||
						(this.settings.direction == 'vertical' && pos >= currItem.posY && pos < currItem.posY + currItem.height)) {
						return currItem;
					}
				}
				return -1;
			...
			_getYPos:function(isScrollbar) {
				var obj = !isScrollbar ? this._dragContainer : this.scrollbarJQ;

				if(!this._useWebkitTransition) {
					return Math.round(obj.position().top);
				} else {

					var transform = obj.css("-webkit-transform");
					var explodedMatrix = transform.replace(/^matrix\(/i, '').split(/, |\)$/g);
					return parseInt(explodedMatrix[5], 10);
				}
			},
			...
			_onDragStart:function(e) {			
				if(!this._isDragging) {		
					...
					this._startPos = this.settings.direction == 'vertical' ? this._getYPos() : this._getXPos();
					
								
					
					this._accelerationX = point.clientX;
					this._accelerationY = point.clientY;
					...
					this._prevMouseY = this._startMouseY = point.clientY;
			...
			_onDragMove:function(e) {
				...
				var point;
				if(this.hasTouch) {
					if(this.settings.direction == 'horizontal' && this._lockYAxis) {
						return false;
					}

					if(this.settings.direction == 'vertical' && this._lockXAxis) {
						return false;
					}
					...
					if(this.settings.direction == 'horizontal' && Math.abs(point.clientY - this._startMouseY) > Math.abs(point.clientX - this._startMouseX) + 3) {
					...
					if(this.settings.direction == 'vertical' && Math.abs(point.clientX - this._startMouseX) > Math.abs(point.clientY - this._startMouseY) + 3) {
						if(this.settings.lockAxis) {
							this._lockXAxis = true;
						}
						return false;
					}
				...
				this._latestDragY = point.clientY;

				// Helps find last direction of drag move
				this._lastDragPosition = this._currentDragPosition;
				var distance = this.settings.direction == 'vertical' ? point.clientY - this._prevMouseY : point.clientX - this._prevMouseX;
				...
				if(distance != 0)
				{
					...
					} else if((this.settings.direction == 'horizontal' && dist <= this.carouselWidth - this._maxXPos) ||
						(this.settings.direction == 'vertical' && dist <= this.carouselHeight - this._maxYPos)) {
					...
					if(this.settings.scrollbar) {
						if (this.settings.direction == 'vertical') {
							this.setYPos((this._scrollbarDist) * (-dist / (this._maxYPos - this.carouselHeight)), true);
						} else {
				...
				this._prevMouseY = point.clientY;
			...
			_onDragRelease:function(e) {
				...
				if(this._isDragging) {		
					...
					this.endPos = this.settings.direction == 'vertical' ? this._getYPos() : this._getXPos();
					...
					//function animate
					if (this.settings.direction == 'vertical') {
						dist = (this._latestDragY - this._accelerationY);
						duration =  Math.max(40, (e.timeStamp || (new Date().getTime())) - this._startTime);
					} else {
						dist = (this._latestDragX - this._accelerationX);
						duration =  Math.max(40, (e.timeStamp || (new Date().getTime())) - this._startTime);
					}
					...
					function getCorrectYPos(pos) {

						if(pos > 0) {
							pos = 0;
						} else if(pos < self.carouselHeight - self._maxYPos) {
							pos = self.carouselHeight - self._maxYPos;
						}
						return pos;
					}
					...
					if(!this.settings.snapToItems) {
						...
						if(this.endPos + S > 0) {	
							if(this.endPos > 0) {
								...
								this.animateTo(
										((this.settings.direction == 'vertical' ? this.carouselHeight : this.carouselWidth) / 10) * ((timeOffset + 200) / 1000),
						...
						} else if(this.settings.direction == 'horizontal' && this.endPos + S < this.carouselWidth - this._maxXPos) {
						...
						} else if(this.settings.direction == 'vertical' && this.endPos + S < this.carouselHeight - this._maxYPos) {
							if(this.endPos < this.carouselHeight - this._maxYPos) {
								this.animateTo(this.carouselHeight - this._maxYPos, 800, "easeOutCubic");
							} else {
								this.animateTo(
										this.carouselWidth - this._maxYPos - (this.carouselHeight / 10) * ((timeOffset + 200) / 1000),
										(Math.abs(this.carouselHeight - this._maxYPos - this.endPos) * 1.1) / v0,
										"easeOutSine",
										true,
										this.carouselHeight - this._maxYPos,
										400,
										"easeOutCubic");
							}
						} else {
						...
						var isNext = this.settings.direction == 'vertical' ? Boolean(this._startMouseY - this._prevMouseY > 0) : Boolean(this._startMouseX - this._prevMouseX > 0);
										
						
						var newX =  getCorrectXPos(this._getXPos());
						var newY =  getCorrectYPos(this._getYPos());
						
											
						var newItemId = this.settings.direction == 'vertical' ? this._getItemAtPos(newY).index : this._getItemAtPos(newX).index;
						...
						if(!this._pagingEnabled) {
							...
							if(isNext) {
								if (this.settings.direction == 'vertical') {
									newY = Math.max(newY - this.carouselHeight - 1, 1 - self._maxYPos);
									newItemId = this._getItemAtPos(newY).index;
								} else {
						...
						if(newItem) {
							...
							if (isNext && this.settings.onNext) {
								this.settings.onNext.call(this,newItemId);
							} else if (!isNext && this.settings.onPrev) {
								this.settings.onPrev.call(this, newItemId);
							}

							newX = getCorrectXPos(-newItem.posX);
							newY = getCorrectYPos(-newItem.posY);

							var newDist,
								newDuration;


							var newDist = Math.abs(this.endPos  - (this.settings.direction == 'vertical' ? newY : newX) );
							...
							if (this.settings.direction == 'vertical') {
								this.animateTo(isFast ? (newY + addDist ) : newY,
	...
	$.fn.touchCarousel.defaults = {  
			...
			snapToItems: true,           // Snaps to items, based on itemsPerMove
			...
			direction: 'horizontal',      // Direction of the items
			...
			itemFallbackHeight: 500,       // Default height of the item in pixels. (used if impossible to get item height).
			
			baseMouseFriction: 0.0012,    // Container friction on desktop (higher friction - slower speed)
			baseTouchFriction: 0.0008,    // Container friction on mobile
			lockAxis: false,               // Allow dragging only on one direction
			...
			onNext: null,                 // Next page handler
			onPrev: null,                 // Prev page handler
